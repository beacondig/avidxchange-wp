<?php
/**
 * Cresta Social Share Counter PRO Meta Box
 */
function cresta_add_meta_box() {
	...
	foreach ( $screens as $screen ) {
		add_meta_box(
			'cresta_sectionid',
			esc_html__( 'Cresta Social Share Counter', 'cresta-social-share-counter-pro' ),
...
function cresta_meta_box_callback( $post ) {
	...
	<label for="cresta_new_field">
        <input type="checkbox" name="cresta_new_field" id="cresta_new_field" value="1" <?php checked( $crestaValue, '1' ); ?> /><?php esc_html_e( 'Hide icons in this page?', 'cresta-social-share-counter-pro' )?>
    </label>
	<?php
}
