...
jQuery(document).ready(function($){   	
	...
	var sliders	=	$('input[type="range"]').length;
	for(var slidercount=0;slidercount<sliders;slidercount++){
		var sliderid	=	($($('input[type="range"]')[slidercount]).attr('id').split('_'))[1];
		var sliderlabel	=	$('#field_'+sliderid).find('label.gfield_label').text();
		calcMergeTagSelect.find('optgroup').eq(0).append( '<option value="{'+sliderlabel+':'+sliderid+'}">'+sliderlabel+'</option>' );
	}
});