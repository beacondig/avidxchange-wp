<?php
/**
 * Template Name: Page - Events (Recruiting)
...
<div class="submenuholder">
	<div class="container-fluid graybg scrollhider">
    	<div class="container mobilescroll event-cat-menu">
        	<a data-event-category="most-recent">Most Recent</a> 
            <?php $stplp = 0; $limlp = 1; ?>
            <?php if( get_field('eventrec') ): ?>
				<?php while( has_sub_field('eventrec') && $stplp == 0 ): ?>
                    <?php // vars
					$stplp = 1;
                    $select = get_sub_field_object('category');
                    $value = get_sub_field('category');

                    foreach( $select['choices'] as $k => $v ): ?>
                    	<?php if($limlp<=5){ ?>
                    		<a class="<?php echo $v; ?>" data-event-category="<?php echo $v; ?>"><?php echo $v; ?></a>&nbsp;
                        <?php } ?>
                        <?php $limlp++; ?>
                    <?php endforeach; wp_reset_postdata(); ?>
                <?php endwhile; wp_reset_query();  ?>
            <?php endif; ?>
            <form id="event-search"><input class="subsearchbox" id="event-search-input" placeholder="Search..." type="text"/><button class="subsearch" type="submit"><i class="fa fa-search" aria-hidden="true"></i></button></form>
        </div>
    </div>
</div>
...
<div class="container"><div class="row event-wrapper" style="font-size:16px; font-weight:300;">
	  <?php 
      $lct = 0;
      $showsign = 0;
      $eventCount = 0;
	  ...
	  $hasREV = get_field('eventrec');
      if( $hasREV ) :
          // loop through the rows of data
		  $repeater = get_field('eventrec');
		  $column_id = array();
			...
			<div class="row center">
				<div>
					<div class="col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1">
						<h4 style="margin:20px;">We don't have any upcoming recruiting events scheduled! Sign up for our email list to be notified about company recruiting events and other news</h4>
    ...
    <div class="container" style="width:100%;">
        <div class="clearfix"></div>
            <div class="col-md-12 center">
                <div class="row">
                    <p>No matching recruiting events found</p>
                </div>
            </div>
        <div class="clearfix"></div>
    </div>
